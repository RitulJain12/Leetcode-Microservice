...
async function validateProblem(req, res, next) {
  try {
    ...
    if (problemfunctionName !== undefined) {
      if (typeof problemfunctionName !== "string" || problemfunctionName.trim().length === 0) {
        return res.status(400).json({ message: "Invalid problemfunctionName" });
      }
      updates.problemfunctionName = problemfunctionName;
    }

    if (functionSignatures !== undefined) {
      if (!Array.isArray(functionSignatures)) {
        return res.status(400).json({ message: "functionSignatures must be an array" });
      }
      updates.functionSignatures = functionSignatures;
    }
